...
- `.github/workflows/collect-prices.yml`：主采集任务，默认每 30 分钟运行一次，排除链动小铺渠道族和 `dujiao` 采集器。
...
```bash
npm run collect:prices -- --all --post --endpoint "$BASE_URL" --exclude-family liandong-shop --exclude-kind dujiao
```
